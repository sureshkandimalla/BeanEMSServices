...
// @beaninfosys.com / @intellanit.com emails get an app token back; every
// other domain is rejected outright, so a forged/edited localStorage entry
// on the frontend can't grant access the way the old client-only auth
// could — AuthFilter re-verifies THIS token's signature on every later
// request instead of trusting anything the client merely claims.
@RestController
